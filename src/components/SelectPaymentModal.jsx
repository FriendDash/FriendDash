...
export default chakra(function SelectPaymentModal({
  isOpen,
  onClose,
  orderId,
  userId,
  ...
  async function onClickSelect() {
    const res = await fetch(
      `https://frienddash-db.onrender.com/orders/${orderId}`,
      {
        method: 'GET',
      }
    );
    if (res.status === 200) {
      ...
      const userOrderDetail = order.orderDetails.find(
        detail => detail.orderUserId === userId
      );
  ...
  async function checkout() {
    ...
    const res = await fetch(
      'https://frienddash-db.onrender.com/stripe/checkout',
      {
        method: 'POST',
        headers: {
    ...
    if (res.status == 200) {
      userOrder.paid = true;
      const res = await fetch(
        `https://frienddash-db.onrender.com/orders/editOrderDetail/${orderId}/${userOrder._id}`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
