...
export default chakra(function RatingPopup(props) {
  ...
  const confirmRatingClick = () => {
    ...
    (async () => {
      const response = await fetch(
        `https://frienddash-db.onrender.com/users/updateUserRating/${props.groupCreatorUserId}/${rating}`,
        {
          method: 'PUT',
          headers: {
  ...
  return (
    <>
      <Button aria-label="Customer checkout" w="130px" onClick={openPopUp}>
        Rate Order
      </Button>
      ...
      >
        <AlertDialogOverlay>
          <AlertDialogContent>
            ...
            <AlertDialogBody>
              <Heading size="md">
                How did {props.groupCreatorName} do? <br />1 = Terrible, 5 =
                Great!
              </Heading>
              <RatingStars
                size={48}
