...
export default chakra(function ViewCardModal({ data, isOpen, onClose }) {
  const {
  ...
  async function deleteCard() {
    const res = await fetch(
      `https://frienddash-db.onrender.com/stripe/paymentMethods/${data.id}`,
      {
        method: 'DELETE',
      }
    );
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose}>
      ...
      <ModalContent marginTop="20%">
        <ModalHeader>
          {styleCardBrand(data.card.brand)} ending in {data.card.last4}
        </ModalHeader>
        <ModalCloseButton />
        ...
        <ModalFooter display={'flex'} justifyContent={'space-between'}>
          ...
          <HStack>
            <Button colorScheme="teal" w="110px" onClick={onEditOpen}>
              Edit Card
            </Button>
            <Button colorScheme="red" w="110px" onClick={onConfirmationOpen}>
              Delete Card
            </Button>
          </HStack>
          <ConfirmationModal
