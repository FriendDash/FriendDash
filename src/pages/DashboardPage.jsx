...
const DashboardPage = () => {
  ...
  const updateList = () => {
    if (!openFilter && !closedFilter && !completedFilter) {
      ...
      var tempOrders = openFilter
        ? ordersFromReduxStore.filter(order => order.orderStatus === 'open')
        : [];
      tempOrders = tempOrders.concat(
        closedFilter
          ? ordersFromReduxStore.filter(order => order.orderStatus === 'closed')
          : []
      );
      tempOrders = tempOrders.concat(
        completedFilter
          ? ordersFromReduxStore.filter(
  ...
  const sortOrders = sortBy => {
    if (sortBy === 'name asc') {
      ...
      setFilteredOrders(
        [...filteredOrders].sort((a, b) => (a.maxSize < b.maxSize ? -1 : 1))
      );
            ...
            >
              ...
              >
                ...
                <option value="members asc">
                  Max. No. of Members Ascending
                </option>
                <option value="members desc">
                  Max. No. of Members Descending
                </option>
              </Select>
            </HStack>
            <SimpleGrid
